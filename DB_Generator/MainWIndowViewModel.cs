...
namespace DB_Generator
{
    class MainWIndowViewModel : INotifyPropertyChanged
    {
        ...
        private  void ExecuteAddCommand()
        {
            ...
            AddDB(data,_Companies,_Customers,_Flights,_Tickets,_Countries);           
        }

        private async void AddDB(DataDB data, int companies,int customers,int flights,int tickets, int countries)
        {
            await Task.Run(() =>
                {
                    if (doWork)
